...
{
	for (auto x: mSites)
		if (url.Equals(x)) return NS_ERROR_FAILURE;
...
nsWebLock::RemoveSite(const nsAString & url)
{
	...
	for (iter = mSites.begin(); iter != mSites.end(); ++iter) {
		if (url.Equals(*iter)) break;
	}
	
	if (iter == mSites.end()) return NS_ERROR_FAILURE;
...
nsWebLock::GetSites(nsIMutableArray * *aSites)
{
	...
	for (auto url: mSites) {
		nsCOMPtr<nsISupportsString> iUrl = do_CreateInstance(NS_SUPPORTS_STRING_CONTRACTID);
		iUrl->SetData(url);
